...
# Output Format
- Return strictly JSON format: `{"status": "success" | "insufficient" | "out_of_domain", "medical_context": "...", "key_points": ["point1", "point2", ...], "reason": "Detailed explanation"}`
...
def info_verifier(state: InfoExtractAgentState):
    ...
    # 결과 파싱 및 로깅
    from app.agents.workflow import clean_and_parse_json
...
def no_results_handler(state: InfoExtractAgentState):
    ...
    {{"status": "out_of_domain" | "insufficient", "reason": "Explain why"}}
